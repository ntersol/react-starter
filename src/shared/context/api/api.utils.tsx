 ...
 */
export const is = {
  entityConfig: <t,>(c: NtsState.ConfigApi<t> | NtsState.ConfigEntity<t>): c is NtsState.ConfigEntity<t> => !!(c as NtsState.ConfigEntity).uniqueId,
...
export const mergePayloadWithApiResponse = <t,>(data2Api: t, dataFromApi: unknown, uniqueId?: string): t => {
  ...
  } else if (typeof dataFromApi === 'object' && !Array.isArray(dataFromApi) && typeof data2Api === 'object' && !Array.isArray(data2Api)) {
 ...
 * @returns
 */
export const arrayToRecord = <t,>(arr: t[], uniqueId: keyof t): Record<string, t> => arr.reduce((a, b) => ({ ...a, [b[uniqueId] as any]: b }), {});
 ...
 * @returns
 */
export const mergeDedupeArrays = <t,>(storeDataSrc: t | t[], payloadSrc: t | t[], uniqueId: keyof t): Partial<NtsState.ApiState> => {
  // Ensure array types
  ...
  const entitiesNew = payload.filter(e => !storeRecord[e[uniqueId] as any]);
 ...
 * @returns
 */
export const deleteEntities = <t,>(storeDataSrc: t | t[], payloadSrc: Partial<t> | Partial<t>[], uniqueId: keyof t): Partial<NtsState.ApiState> => {
  // Ensure array types
  const storeData = !Array.isArray(storeDataSrc) ? [storeDataSrc] : storeDataSrc;
  const payload = !Array.isArray(payloadSrc) ? [payloadSrc] : payloadSrc;
  // Convert both types to records
